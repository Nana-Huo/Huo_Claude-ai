"""
汉中市胆结石风险调研 - 多因素Logistic回归分析（使用卡方检验筛选的特征）
...
class LogisticRegressionAnalysis:
    def __init__(self, data_path, target_column=None, selected_features=None):
    ...
    def load_data(self):
        ...
        try:
            ...
            if self.selected_features:
                # 确保目标变量也在数据中
                columns_to_use = self.selected_features + [self.target_column]
                self.data = self.data[columns_to_use]
                print(f"使用筛选的特征: {len(self.selected_features)} 个")
    ...
    def fit_model(self):
        ...
        try:
            self.results = logit_model.fit(disp=False, maxiter=100)
            print("模型拟合成功")
            return True
        except Exception as e:
            print(f"模型拟合失败: {e}")
...
- 特征数量: {len(self.feature_names)}（从卡方检验筛选）
...
"""
        ...
        report += f"""
结论:
通过多因素Logistic回归分析，在卡方检验筛选的 {len(self.feature_names)} 个特征中，
共识别出 {len(self.significant_features)} 个与胆结石发病显著相关的独立风险因素。
...
if __name__ == "__main__":
    ...
    try:
        chi_square_results = pd.read_csv(
            r"C:\Users\霍冠华\Documents\trae_projects\claude code\卡方检验_显著特征.csv",
            encoding='utf-8-sig'
        )
        selected_features = chi_square_results['feature'].tolist()
        print(f"从卡方检验结果中加载了 {len(selected_features)} 个显著特征")
    except:
        print("未找到卡方检验结果文件，将使用所有特征")
